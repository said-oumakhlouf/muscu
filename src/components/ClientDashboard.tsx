        ...
        <div className="w-full bg-[#cec4e2] px-6 py-24">
            <div className="max-w-4xl mx-auto flex flex-col gap-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <StatCard icon={<CheckCircle size={20} className="text-[#7C5CBF]" />} value={completed.length} label="Séances" sub="complétées" />
                    <StatCard icon={<Flame size={20} className="text-[#7C5CBF]" />} value={streak > 0 ? `${streak}j` : '—'} label="Streak" sub="jours consécutifs" />
                    <StatCard icon={<Zap size={20} className="text-[#7C5CBF]" />} value={totalCalories > 0 ? `${totalCalories}` : '—'} label="Calories" sub="brûlées" />
                    <StatCard
                        icon={<Calendar size={20} className="text-[#7C5CBF]" />}
                        value={nextSession?.scheduledAt ? new Date(nextSession.scheduledAt).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' }) : '—'}
                        label="Prochaine"
                        sub={nextSession?.scheduledAt ? new Date(nextSession.scheduledAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }) : 'aucune prévue'}
                    />
                </div>

                {/* Progression */}
                <div className="flex flex-col gap-3">
                    <h2 className="text-xs font-bold uppercase tracking-widest text-[#7C5CBF]">Progression</h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="p-6 rounded-2xl bg-white border border-[#E8DEFF] shadow-sm flex flex-col items-center justify-center gap-3">
                            ...
                            <div className="relative flex items-center justify-center">
                                <ProgressRing value={completed.length} max={MONTHLY_GOAL} size={120} />
                                <div className="absolute flex flex-col items-center">
                                    <span className="text-2xl font-black text-[#1A1A2E]">{completed.length}</span>
                                    <span className="text-xs text-[#9CA3AF]">/ {MONTHLY_GOAL}</span>
                                </div>
                            </div>
                            <p className="text-xs text-[#9CA3AF] text-center">
                                {completed.length >= MONTHLY_GOAL ? '🎉 Objectif atteint !' : `${MONTHLY_GOAL - completed.length} séance${MONTHLY_GOAL - completed.length > 1 ? 's' : ''} restante${MONTHLY_GOAL - completed.length > 1 ? 's' : ''}`}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                {/* Historique */}
                <div className="flex flex-col gap-3">
                    <h2 className="text-xs font-bold uppercase tracking-widest text-[#7C5CBF]">Historique</h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <RecentSessionsCard sessions={recentSessions} />
                        <MuscleGroupsCard muscleCount={muscleCount} totalExercises={totalExercises} />
                    </div>
                </div>
