...
export default function RegisterStepper({
  ...
  const [role, setRole] = useState<"client" | "coach" | null>(
    defaultRole ?? null,
  );
  ...
  const isStepValid = () => {
    ...
    if (step === 1) {
      return (
        ...
        passwordRules.every((r) => r(form.password))
      );
    }
    if (role === "client") {
      if (step === 2)
  ...
  return (
    <div className="bg-white rounded-2xl border border-black/6 p-8 w-full max-w-md">
      ...
      <div className="mt-6">
        ...
        {role === "coach" && step === 3 && (
          <StepCoachProfile form={form} update={update} />
        )}
      </div>
      ...
      {step > 0 && (
        <div className="flex gap-3 mt-6">
          <button
            onClick={() =>
              step === 1 ? (setStep(0), setRole(null)) : setStep(step - 1)
            }
