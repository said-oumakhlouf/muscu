...
import { MuscleIcon } from "@/utils/muscleGroupConfig";
...
export default function SessionExerciseList({
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    const muscleGroup = WGER_CATEGORY_TO_MUSCLE[exercise.category] || "";
    setPendingExercise({ ...exercise, muscleGroup });
  };
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
      ...
      {isCoach && (
        <div
          ...
          onDrop={handleDrop}
          className={`border-2 border-dashed rounded-xl p-4 mb-4 text-center transition ${
            isDragOver
      ...
      {exercises.length === 0 ? (
        <div className="text-center py-12 text-gray-400">
          ...
          {isCoach && (
            <p className="text-xs mt-1">
              Glisse des exercices depuis la bibliothèque
            </p>
          )}
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {exercises.map((se) => (
            <div
              key={se.id}
              className="flex items-center gap-3 px-4 py-3 bg-zinc-50 rounded-xl border border-gray-100"
            >
              <div className="w-8 h-8 shrink-0">
                <MuscleIcon muscleGroup={se.exercise.muscleGroup} size={18} />
              </div>
