...
export default function SessionDetailPage() {
    ...
    const handleDrop = async (exercise: { externalId: string; name: string }, sets: number, reps: number, weight?: number) => {
        if (!token || !session) return;

        try {
            const data = await fetchWithAuth('http://localhost:3000/exercises/find-or-create', token, {
                method: 'POST',
                body: JSON.stringify({
                    name: exercise.name,
            ...
            await sessionService.addExercise(token, sessionId, { exerciseId: data.id, sets, reps, weight });
